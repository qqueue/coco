...
exports.tokenize = function(code, o){
  var c, that;
  this.code = code;
  this.inter || (this.code = this.code.replace(/[\r\u2028\u2029\uFEFF]/g, ''));
  this.code = '\n' + this.code;
  ...
  this.flags = [];
  this.i = 0;
  while (c = this.code.charAt(this.i)) {
    this.advance((fn$.call(this)));
  }
  ...
  function fn$(){
    switch (c) {
    case ' ':
      return this.doSpace(this.code, this.i);
    case '\n':
      return this.doLine(this.code, this.i);
    case '\\':
      return this.doBackslash(this.code, this.i);
    case '\'':
    case '"':
      return this.doString(this.code, this.i, c);
    case "0":
    case "1":
    case "2":
    case "3":
    ...
    case "8":
    case "9":
      return this.doNumber(this.code, this.i);
    case '/':
      switch (this.code.charAt(this.i + 1)) {
      case '*':
        return this.doComment(this.code, this.i);
      case '/':
        return this.doHeregex(this.code, this.i);
      default:
        return this.doRegex(this.code, this.i) || this.doLiteral(this.code, this.i);
      }
      break;
    case '`':
      return this.doJS(this.code, this.i);
    default:
      return this.doID(this.code, this.i) || this.doLiteral(this.code, this.i) || this.doSpace(this.code, this.i);
    }
  }
};
exports.dent = 0;
...
exports.token = function(tag, value, callable){
  this.tokens.push(this.last = [tag, value, this.line, this.loc(value)]);
  if (callable) {
    this.last.callable = true;
  }
  return value;
};
exports.loc = function(it){
  var lines, lastColumn, x0$;
  lines = 0;
  lastColumn = 0;
  if (typeof it === 'string') {
    x0$ = it.split(/\n/);
    lines = x0$.length - 1;
    lastColumn = x0$.length > 1
      ? x0$[x0$.length - 1].length
      : this.column + it.length;
  }
  return {
    first_line: -~this.line,
    first_column: this.column,
    last_line: -~this.line + lines,
    last_column: lastColumn || this.column
  };
};
...
exports.advance = function(chars){
  var consumed, lines;
  consumed = this.code.substr(this.i, chars);
  if ((lines = consumed.split(/\n/)).length > 1) {
