...
function printTokens(tokens){
  var lines, i$, len$, ref$, tag, val, lno, loc, x0$, j$, len1$, l;
  lines = [];
  for (i$ = 0, len$ = tokens.length; i$ < len$; ++i$) {
    ref$ = tokens[i$], tag = ref$[0], val = ref$[1], lno = ref$[2], loc = ref$[3];
    (lines[lno] || (lines[lno] = [])).push((tag.toLowerCase() === val
      ? tag
      : tag + ":" + val) + "" + [loc ? (x0$ = loc, "[" + x0$.first_line + ":" + x0$.first_column + "-" + x0$.last_line + ":" + x0$.last_column + "]") : void 8]);
  }
